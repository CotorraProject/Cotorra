...
{
    public class RuleValidator<T> where T : BaseEntity
    {
        public void ValidateRules(List<T> lstObjectsToValidate, List<IValidationRule> rules)
        {
            List<RuleValidationReg> validations = new List<RuleValidationReg>();
            List<RuleValidation> res = new List<RuleValidation>();
            for (int i = 0; i < rules.Count; i++)
            {
                var validationsRes = rules[i].ExecuteValidation(lstObjectsToValidate.Cast<BaseEntity>().ToList());
