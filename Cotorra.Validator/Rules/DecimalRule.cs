...
{
    public class  DecimalRule: IValidationRule
    {
        ...
        public DecimalRule(string fieldName, string friendlyFieldName,  decimal min, decimal max, int erorCode)
        {
            this.Max = max;
            this.Min = min; 
        ...
        public Tuple<bool, List<RuleValidation>> ExecuteValidation(List<BaseEntity> ToValidate)
        {
            ...
            for (int i = 0; i < ToValidate.Count; i++)
            {
                ...
                var value = Convert.ToDecimal(fld.GetValue(obj));
