...
{
    public class DiasInfonavitPorcentajeAnteriorFunction : FunctionExtension
    {
        #region "Attributes"
        ...
        #region "Constructor"
        public DiasInfonavitPorcentajeAnteriorFunction(FunctionParams functionParams)
        {
        ...
        public double calculate()
        {
            ...
            //Restar días de incidencias e incapacidades
            ...
            if (historicEmployeeSalaryAdjustments.Any())
            {
                ...
                if (modificationDate.Any())
                {
                    ...
                    result = (modificationDate.FirstOrDefault().AddDays(-1) - initialDate).TotalDays - entryDays;
                    ...
                    {
                        if (modificationDate.FirstOrDefault().AddDays(-1) > inha.InitialDate)
                        {
                            inhabilityTotalDays += new DateTimeUtil().InclusiveDays(
                                inha.InitialDate, modificationDate.FirstOrDefault().AddDays(-1),
        ...
        {
            return new DiasInfonavitPorcentajeAnteriorFunction(_functionParams);
        }
