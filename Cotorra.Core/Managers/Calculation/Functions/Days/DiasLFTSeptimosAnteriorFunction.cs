...
{
    public class DiasLFTSeptimosAnteriorFunction : FunctionExtension
    {
        #region "Attributes"
        ...
        #region "Constructor"
        public DiasLFTSeptimosAnteriorFunction(FunctionParams functionParams)
        {
        ...
        public double calculate()
        {
            ...
            if (historicEmployeeSalaryAdjustments.Any())
            {
                ...
                {
                    //Un día antes de la modificación de salario, debido a que el filtro es incluyente
                    var modificationDateSelected = modificationDate.LastOrDefault().AddDays(-1);
                    ...
                    foreach (var split in splitted)
                    {
                        ...
                        if (splitInt != -1)
                        {
                            result += DateTimeUtil.CountDays((DayOfWeek)splitInt, periodInitialDate, modificationDateSelected);
                        }
                    }
                }
            }
        ...
        {
            return new DiasLFTSeptimosAnteriorFunction(_functionParams);
        }
