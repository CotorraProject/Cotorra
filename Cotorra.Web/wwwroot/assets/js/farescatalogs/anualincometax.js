...
UX.Cotorra.AnualIncomeTax = {

    UI: {

        CatalogName: 'AnualIncomeTax',
        ContainerSelector: '#np-farescatalogs',
        TitleModalsString: 'ISR anual',
        ...
        LoadGrid: function (data = [], setValidity = true) {
            ...
            if (data.length > 0 && setValidity) {
                ...
                $drpValidity.off('change').on('change', function () { UX.Cotorra.AnualIncomeTax.UI.LoadGrid([data[parseInt($(this).val())]], false); });
                $drpValidity.change();
                return;
